...
class IceInjector:
    ...
    def __init__(self, ice_patch_folder):
        ...
        # Save n of variants
        self.variants = len(self.ice_patch_paths)
    ...
    def inject(self, original_img_path, out_folder):
        ...
        for patch_path in self.ice_patch_paths:
            logging.info(f"Applying Ice Patch: {os.path.basename(patch_path)}")
            ...
            out_dir = FilePathManager.getVariantOutputFolder(out_folder, self.ice_patch_paths.index(patch_path))
