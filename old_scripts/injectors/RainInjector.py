...
class RainInjector:
    ...
    def __init__(self, rain_patch_folder):
        ...
        # Save n of variants
        self.variants = len(self.rain_patch_paths)
    ...
    def inject(self, original_img_path, out_folder):
        ...
        for patch_path in self.rain_patch_paths:
            logging.info(f"Applying Rain Patch: {os.path.basename(patch_path)}")
            ...
            out_dir = FilePathManager.getVariantOutputFolder(out_folder, self.rain_patch_paths.index(patch_path))
